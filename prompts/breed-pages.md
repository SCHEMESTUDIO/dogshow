...
1. `data/breed-page-queue.md` — the queue. Take the next 2 PENDING entries (Phase A top-to-bottom, then Phase B). Respect every SKIP/BLOCKED note.
...
THIS RUN — for each of the 2 breeds:
...
5. `sitemap.xml`: add `https://dogshow.lol/breeds/{slug}` following the existing entry format.
...
- Rough word count: your entry's copy fields total ≥900 words (count them).
