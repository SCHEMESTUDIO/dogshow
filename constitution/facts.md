...
## Other authors
- breed-pages.yml — daily 06:00 UTC cron — Claude writes AND PUBLISHES 2 new /breeds/{slug} pages to prod (api/breed.js, breeds.js, breeds.html, sitemap.xml, llms.txt, data/breed-page-queue.md) — STAGED, NOT YET LIVE: the workflow file is uncommitted in the local repo as of 2026-07-19 (live sitemap static at 19 breeds confirms no runs); it ACTIVATES when the Dog Show repo is next pushed — a deliberate James decision, not a side effect. M2 must never write breed content regardless (territory split, seo.json content_policy). <!-- src: dogshow git status 2026-07-19; live sitemap -->
...
## URLs / stack
- Routes: / · /show · /dogs · /d/{slug} (dog certificate) · /breeds + /breeds/{slug} (19 live guides, +2/day via CI) · /leaderboard · /resources · /about · /terms · /privacy · /llms.txt. <!-- src: vercel.json rewrites; llms.txt; breeds.html -->
