...
5. **Public read** — `GET /testimonials` (5-min `Cache-Control`) returns approved entries; `index.html` fetches on load. **Section is hidden via `display: none` until at least one approved testimonial arrives** (better than an empty rail or placeholder).
...
| `party/` | PartyKit server code — single `server.js` (~3753 lines, one class), has its own node_modules. NOT deployed to Vercel (`.vercelignore`). |
