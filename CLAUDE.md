...
**Geo scope (EU-only):** `middleware.mjs` classifies each request via Vercel geolocation and sets `dogshow_geo=EU|ROW`. EEA + UK + Switzerland → `EU` → banner shown, trackers gated. Everyone else → `ROW` → trackers load with no banner (US, etc. — opt-out regimes). Decision was deliberate (James, 2026-06-04): preserve US ad-conversion data while complying where opt-in is required. **Fail-safe:** missing/unknown geo is treated as `EU`.
